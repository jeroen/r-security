...
\Abstract{
  ...
  scientific super computers bring great potential for making \R accessible
  ...
  machine through the interactive console. As a result the execution
  environment of \R is entirely unrestricted, which could potentially result in
...
change the role of \R from and end-user tool towards a general purpose computational 
back-end, powering systems and software stacks with high quality analytics and visualization. 

However, one reason developers are still reluctant to build on \R are concerns
...
from embedding other software or languages. Properly addressing these challenges
could facilitate wider adoption of \R as a general purpose computational back-end engine.
...
methods of securing \R. Different aspects and concerns of
...
As it turns out, \R itself is not very suitable for implementing
...
is used throughout the paper to show one way of addressing issues.
...
use \R other purposes to join the discussion and share their
...
flexible and unobtrusive enough to not interfere with our daily workflow, we could
even make a habit out of using it every time we use contributed code (which to
...
inside various systems and web services that provide on-demand calculations and
...
second. Furthermore the systems have to be stable, requiring that jobs should
...
per process basis. These requirements demands a more flexible and finer degree
...
tools (user separation, chroot, etc) and has been the most central motivation of this research.
...
security exploit, or which searches the system for sensitive personal information.
...
The other category of problems is not so necessarily related to deliberate abuse, and
...
However, when \R is embedded in a bigger system, the situation is
more complicated, and we have to think about these scenarios in advance. If
an out-of-control \R job is not properly detected and terminated, the process
might very well run forever and take down our service, or even the entire machine. This has
...
time to process is proportional to the size of the data, i.e.\ the number
...
Statistical computing is full of such intrinsic edge-cases and unexpected caveats.
 Even when only using tested code or
predefined services, this does not entirely guarantee smooth and timely
...
aspect and have systems in place to manage processes without
manual intervention.
...
An alternative method to implement sanitizing is to define an extensive blacklist of
...
we assume he has done a good job. However, it makes it hard to maintain and
cumbersome to generalize the approach to other \R packages (by default the
method does not allow loading other packages). Furthermore the entire method
falls if  one function has been overlooked or changes between
versions, which does make the method quite vulnerable. In \R even the most
fundamental primitive functions can be exploited to circumvent namespace
masking, so it is unwise to rely solely on blacklisting listing for security. 
...
However, when used for example in combination with the \RAppArmor package, 
something like \pkg{sandboxR} could be useful to detect problematic calls earlier on and present more informative errors naming a specific forbidden functions rather than just "permission denied".
...
\RAppArmor (see also section section \ref{usr.bin.r}). However, this makes it a bit limited
and usually overly restrictive. We want more flexible that set different policies, priorities and restrictions for different users or tasks.
...
to be avoided for security reasons. This enables a new approach to socialize scientific computing and lies at the core of the \texttt{OpenCPU} framework \citep{opencpu}, which exposes a public HTTP API to run and share \R code on a central server.
...
box'' on Ubuntu 12.04 (Precise) and up. It has also successfully been tested on Debian
...
build on OpenSuse 12.1 and up. Note that Suse systems organize the file system in a
slightly different way than Ubuntu and Debian, so the profiles need to be
modified accordingly. The root directory of the \RAppArmor source package contains a text file with some specific instructions for Debian and OpenSuse.
...
how to create a working sandbox in \R using \AppArmor. The \RAppArmor
...
profiles. On Ubuntu it is easiest installed using binary \texttt{deb} packages provided through launchpad:
...
associate a process with a security profile in a way that does allow to escape
...
defined in the \texttt{tunables/global} include that ships with \AppArmor and matches the location of the user home directory, i.e.\ \texttt{/home/jeroen} for a user named ``jeroen''.
...
in 5 seconds. Below is a snippet that will take much more than 5 seconds to complete on most
